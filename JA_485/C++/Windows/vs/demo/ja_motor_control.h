...
public:
    ...
    void write_time_position(int initial_positions, int target_positions, double Hz = 150, double duration = 4, int num_points = 5000);
...
private:
    ...
    {
        DWORD bytesWritten;
        DWORD size_ = static_cast<DWORD>(size);
        if (!WriteFile(hSerial, data, size_, &bytesWritten, NULL))
        {
            throw runtime_error("Serial port writing failed!");
        }

        if (bytesWritten != size)
        {
            throw runtime_error("The written data is incomplete!");
        }
    }
    ...
    vector<uint8_t> receive_data(size_t expected_size, uint16_t timeout_ms = 1000)
    {
        ...
        while (bytesRead < expected_size)
        {
            auto current_time = chrono::steady_clock::now();
            auto elapsed_time = chrono::duration_cast<chrono::milliseconds>(current_time - start_time).count();
            if (elapsed_time >= timeout_ms)
            {
                throw runtime_error("Read timed out!");
            }

            DWORD tempRead = 0;
            DWORD chunkSize = static_cast<DWORD>(expected_size - bytesRead);
            if (!ReadFile(hSerial, buffer.data() + bytesRead, chunkSize, &tempRead, NULL))
            {
